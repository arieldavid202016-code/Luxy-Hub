...
local CoreGui = SharedServices.CoreGui
local Players = SharedServices.Players
local RunService = SharedServices.RunService
local SoundService = SharedServices.SoundService
local UserInputService = SharedServices.UserInputService
local TextService = SharedServices.TextService
local Teams = SharedServices.Teams
local TweenService = SharedServices.TweenService
...
local Library = {
	...
	Scheme = {
		...
		AccentColor = Color3.fromRGB(220, 30, 45),
		...
		DarkColor = Color3.fromRGB(27, 27, 27),
		WhiteColor = Color3.new(0.92549, 0.92549, 0.92549),
	},
	Registry = {},
	Scales = {},
...
if RunService:IsStudio() then
	...
	Library.OriginalMinSize = Library.IsMobile
		and Vector2.new(480, 240)
...
Templates.Window = {
	Title = "Luxy Hub",
	Icon = "rbxassetid://119236006737744",
...
local function CheckDepbox(Box, Search)
	...
	for _, ElementInfo in Box.Elements do
		...
		repeat
			...
			__DARKLUA_CONTINUE_3 = true
		until true
		if not __DARKLUA_CONTINUE_3 then break end
	end
	for _, Depbox in Box.DependencyBoxes do
		local __DARKLUA_CONTINUE_4 = false
		repeat
			if not Depbox.Visible then
				__DARKLUA_CONTINUE_4 = true
				break
			end
			VisibleElements = VisibleElements + CheckDepbox(Depbox, Search)
			__DARKLUA_CONTINUE_4 = true
		until true
		if not __DARKLUA_CONTINUE_4 then break end
	end
			...
			for _, SubTab in InnerTabbox.Tabs do
				...
				for _, ElementInfo in SubTab.Elements do
					...
					repeat
						...
						__DARKLUA_CONTINUE_7 = true
					until true
					if not __DARKLUA_CONTINUE_7 then break end
				end
				for _, Depbox in SubTab.DependencyBoxes do
					local __DARKLUA_CONTINUE_8 = false
					repeat
						if not Depbox.Visible then
							__DARKLUA_CONTINUE_8 = true
							break
						end
						InnerVisibleElements[SubTab] = InnerVisibleElements[SubTab] + CheckDepbox(Depbox, Search)
						__DARKLUA_CONTINUE_8 = true
					until true
					if not __DARKLUA_CONTINUE_8 then break end
				end
			end
			for SubTab, Visible in InnerVisibleElements do
				...
				if Visible > 0 then
					...
					if InnerTabbox.ActiveTab == SubTab then
						SubTab:Resize()
					elseif InnerTabbox.ActiveTab
						and InnerVisibleElements[InnerTabbox.ActiveTab] == 0
					then
						SubTab:Show()
					end
				end
...
local function RestoreDepbox(Box)
	for _, ElementInfo in Box.Elements do
		ElementInfo.Holder.Visible = (
			typeof(ElementInfo.Visible) == "boolean"
			and ElementInfo.Visible
			or true
		)
	...
	for _, Depbox in Box.DependencyBoxes do
		local __DARKLUA_CONTINUE_11 = false
		repeat
			if not Depbox.Visible then
				__DARKLUA_CONTINUE_11 = true
				break
			end
			RestoreDepbox(Depbox)
			__DARKLUA_CONTINUE_11 = true
		until true
		if not __DARKLUA_CONTINUE_11 then break end
	end
	if Box.InnerTabboxes then
		for _, InnerTabbox in Box.InnerTabboxes do
			for _, SubTab in InnerTabbox.Tabs do
				...
				for _, Depbox in SubTab.DependencyBoxes do
					...
					repeat
						...
						__DARKLUA_CONTINUE_15 = true
					until true
					if not __DARKLUA_CONTINUE_15 then break end
				end
...
local function ApplySearchToTab(Tab, Search)
	...
	for _, Groupbox in Tab.Groupboxes do
		...
		for _, ElementInfo in Groupbox.Elements do
			local __DARKLUA_CONTINUE_17 = false
			repeat
				if ElementInfo.Type == "Divider" then
					ElementInfo.Holder.Visible = false
					__DARKLUA_CONTINUE_17 = true
					break
				elseif ElementInfo.SubButton then
					local Visible = false
					if string.find(ElementInfo.Text:lower(), Search, 1, true)
				...
				__DARKLUA_CONTINUE_17 = true
			until true
			if not __DARKLUA_CONTINUE_17 then break end
		end
		for _, Depbox in Groupbox.DependencyBoxes do
			...
			repeat
				...
				__DARKLUA_CONTINUE_18 = true
			until true
			if not __DARKLUA_CONTINUE_18 then break end
		end
			...
			for _, SubTab in InnerTabbox.Tabs do
				...
				for _, ElementInfo in SubTab.Elements do
					...
					repeat
						...
						__DARKLUA_CONTINUE_21 = true
					until true
					if not __DARKLUA_CONTINUE_21 then break end
				end
				for _, Depbox in SubTab.DependencyBoxes do
					local __DARKLUA_CONTINUE_22 = false
					repeat
						if not Depbox.Visible then
							__DARKLUA_CONTINUE_22 = true
							break
						end
						InnerVisibleElements[SubTab] = InnerVisibleElements[SubTab] + CheckDepbox(Depbox, Search)
						__DARKLUA_CONTINUE_22 = true
					until true
					if not __DARKLUA_CONTINUE_22 then break end
				end
			end
			for SubTab, Visible in InnerVisibleElements do
				SubTab.ButtonHolder.Visible = Visible > 0
	...
	for _, Tabbox in Tab.Tabboxes do
		...
		for _, SubTab in Tabbox.Tabs do
			...
			for _, ElementInfo in SubTab.Elements do
				local __DARKLUA_CONTINUE_26 = false
				repeat
					if ElementInfo.Type == "Divider" then
						ElementInfo.Holder.Visible = false
						__DARKLUA_CONTINUE_26 = true
						break
					elseif ElementInfo.SubButton then
						local Visible = false
						if string.find(ElementInfo.Text:lower(), Search, 1, true)
					...
					__DARKLUA_CONTINUE_26 = true
				until true
				if not __DARKLUA_CONTINUE_26 then break end
			end
			for _, Depbox in SubTab.DependencyBoxes do
				...
				repeat
					...
					__DARKLUA_CONTINUE_27 = true
				until true
				if not __DARKLUA_CONTINUE_27 then break end
			end
		end
		for SubTab, Visible in VisibleElements do
			...
			if Visible > 0 then
				...
				if Tabbox.ActiveTab == SubTab then
					SubTab:Resize()
				elseif Tabbox.ActiveTab
					and VisibleElements[Tabbox.ActiveTab] == 0
				then
					SubTab:Show()
				end
			end
...
local function ResetTab(Tab)
	...
	for _, Groupbox in Tab.Groupboxes do
		for _, ElementInfo in Groupbox.Elements do
			...
			if ElementInfo.SubButton then
				ElementInfo.Base.Visible = ElementInfo.Visible
				ElementInfo.SubButton.Base.Visible =
					ElementInfo.SubButton.Visible
			end
		end
		for _, Depbox in Groupbox.DependencyBoxes do
			local __DARKLUA_CONTINUE_31 = false
			repeat
				if not Depbox.Visible then
					__DARKLUA_CONTINUE_31 = true
					break
				end
				RestoreDepbox(Depbox)
				__DARKLUA_CONTINUE_31 = true
			until true
			if not __DARKLUA_CONTINUE_31 then break end
		end
		Groupbox:Resize()
...
function Library:UpdateSearch(SearchText)
	...
	elseif Library.LastSearchTab
		and typeof(Library.LastSearchTab) == "table"
	then
	...
	if not Library.GlobalSearch
		and Library.ActiveTab
		and Library.ActiveTab.IsKeyTab
	then
		Library.Searching = false
		Library.LastSearchTab = nil
		return
...
function Library:GiveSignal(
	Connection
)
	local ConnectionType = typeof(Connection)
	if Connection
		and (
			ConnectionType == "RBXScriptConnection"
			or ConnectionType == "RBXScriptSignal"
		)
	then
...
function Library:Validate(
	...
	for k, v in Template do
		local __DARKLUA_CONTINUE_44 = false
		repeat
			if typeof(k) == "number" then
				__DARKLUA_CONTINUE_44 = true
				break
			end
			...
			__DARKLUA_CONTINUE_44 = true
		until true
		if not __DARKLUA_CONTINUE_44 then break end
	end
	return Table
end

local function FillInstance(
	Table,
	Instance
)
...
do
	...
	table.insert(
		Library.Scales,
		New("UIScale", { Parent = NotificationArea })
	)
...
function Library:GetKeyString(KeyCode)
	if KeyCode.EnumType == Enum.KeyCode
		and KeyCode.Value > 33
		and KeyCode.Value < 127
	then
...
function Library:GetTextBounds(
	Text,
	Font,
	Size,
	Width
	...
	Params.Size = Size
	Params.Width = Width
		or workspace.CurrentCamera.ViewportSize.X - 32
	local Bounds = TextService:GetTextBoundsAsync(Params)
	return Bounds.X, Bounds.Y
end

function Library:MouseIsOverFrame(
	Frame,
	Mouse
)
...
function Library:MakeDraggable(
	...
	Library:GiveSignal(
		UserInputService.InputChanged:Connect(function(Input)
			if (not IgnoreToggled and not Library.Toggled)
				or (IsMainWindow and Library.CantDragForced)
	...
	function Table:Open()
		...
		if typeof(Offset) == "function" then
			Menu.Position = UDim2.fromOffset(
				...
				math.floor(
					Holder.AbsolutePosition.Y + Offset()[2]
				)
			)
		else
			Menu.Position = UDim2.fromOffset(
		...
		Menu.Size = typeof(Table.Size) == "function"
			and Table.Size()
			or Table.Size
		...
		Menu.Visible = true
		Table.Signal = Holder:GetPropertyChangedSignal(
			"AbsolutePosition"
		):Connect(function()
			if typeof(Offset) == "function" then
				Menu.Position = UDim2.fromOffset(
...
Library:GiveSignal(
	UserInputService.InputBegan:Connect(function(Input)
		...
		if IsClickInput(Input, true) then
			...
			if CurrentMenu
				and not (
					Library:MouseIsOverFrame(
						CurrentMenu.Menu,
						Location
					)
					or Library:MouseIsOverFrame(
						CurrentMenu.Holder,
						Location
					)
				)
...
table.insert(
	Library.Scales,
	New("UIScale", { Parent = TooltipLabel })
)
TooltipLabel:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
	...
	local X, _ = Library:GetTextBounds(
		...
		(
			...
			- TooltipLabel.AbsolutePosition.X
			- 8
		) / Library.DPIScale
	)
...
function Library:AddTooltip(
	...
	local function DoHover()
		if CurrentHoverInstance == HoverInstance
			or Library.ActiveDialog
			or (
				CurrentMenu
				and Library:MouseIsOverFrame(
					CurrentMenu.Menu,
					Mouse
				)
			)
			or (
				TooltipTable.Disabled
				and typeof(DisabledInfoStr) ~= "string"
			)
			or (
				not TooltipTable.Disabled
				and typeof(InfoStr) ~= "string"
			)
		then
			return
		end
		CurrentHoverInstance = HoverInstance
		TooltipLabel.Text = TooltipTable.Disabled
			and DisabledInfoStr
			or InfoStr
		TooltipLabel.Visible = true
		while Library.Toggled
...
do
	...
	function Funcs:AddKeyPicker(Idx, Info)
		...
		if KeyPicker.Mode == "Press" then
			assert(
				ParentObj.Type == "Label",
				"KeyPicker with the mode 'Press' can be only applied on Labels."
			)
		...
		local GetActiveModifiers = function()
			...
			for Name, Input in Modifiers do
				local __DARKLUA_CONTINUE_51 = false
				repeat
					if table.find(ActiveModifiers, Name) then
						__DARKLUA_CONTINUE_51 = true
						break
					end
					...
					__DARKLUA_CONTINUE_51 = true
				until true
				if not __DARKLUA_CONTINUE_51 then break end
			end
			return ActiveModifiers
		end
		local AreModifiersHeld = function(Required)
			if not (
					typeof(Required) == "table"
					and GetTableSize(Required) > 0
				) then
				return true
			end
			...
			for _, Name in Required do
				local __DARKLUA_CONTINUE_52 = false
				repeat
					if table.find(ActiveModifiers, Name) then
						__DARKLUA_CONTINUE_52 = true
						break
					end
					Holding = false
					break
				until true
				if not __DARKLUA_CONTINUE_52 then break end
			end
			return Holding
		end
		...
		local VerifyModifiers = function(CurrentModifiers)
			...
			for _, name in CurrentModifiers do
				local __DARKLUA_CONTINUE_54 = false
				repeat
					if not Modifiers[name] then
						__DARKLUA_CONTINUE_54 = true
						break
					end
					table.insert(ValidModifiers, name)
					__DARKLUA_CONTINUE_54 = true
				until true
				if not __DARKLUA_CONTINUE_54 then break end
			end
		...
		do
			...
			local CheckImage = New("ImageLabel", {
				...
				ImageRectOffset = CheckIcon
					and CheckIcon.ImageRectOffset
					or Vector2.zero,
				ImageRectSize = CheckIcon
					and CheckIcon.ImageRectSize
					or Vector2.zero,
				ImageTransparency = 1,
		...
		function KeyPicker:Update()
			...
			if KeyPicker.Mode == "Toggle"
				and ParentObj.Type == "Toggle"
				and ParentObj.Disabled
			then
				KeybindsToggle:SetVisibility(false)
				return
			end
			local State = KeyPicker:GetState()
			local ShowToggle = Library.ShowToggleFrameInKeybinds
				and KeyPicker.Mode == "Toggle"
			if KeyPicker.SyncToggleState
				and ParentObj.Value ~= State
			then
		...
		function KeyPicker:DoClick()
			if KeyPicker.Mode == "Press" then
				if KeyPicker.Toggled
					and Info.WaitForCallback == true
				then
					return
				end
		...
		Picker.MouseButton1Click:Connect(function()
			...
			elseif Input.UserInputType == Enum.UserInputType.Keyboard then
				Key = Input.KeyCode == Enum.KeyCode.Escape
					and "None"
		...
		Library:GiveSignal(
			UserInputService.InputBegan:Connect(function(Input)
				...
				local Key = KeyPicker.Value
				local HoldingModifiers = AreModifiersHeld(
					KeyPicker.Modifiers
				)
		...
		KeyPicker.DefaultModifiers = table.clone(
			KeyPicker.Modifiers or {}
		)
		KeyPicker.Idx = Idx
	...
	function Funcs:AddColorPicker(Idx, Info)
		...
		local ColorPicker = {
			Value = Info.Default,
			Transparency = (
				typeof(Info.Transparency) == "number"
				and Info.Transparency
			) or 0,
			Title = Info.Title,
			Callback = Info.Callback,
			Changed = Info.Changed,
		...
		ColorPicker.Hue, ColorPicker.Sat, ColorPicker.Vib =
			ColorPicker.Value:ToHSV()
		local Holder = New("TextButton", {
			BackgroundColor3 = ColorPicker.Value,
			BorderColor3 = Library:GetDarkerColor(
				ColorPicker.Value
			),
			BorderSizePixel = 1,
			Size = UDim2.fromOffset(18, 18),
		...
		local HolderTransparency = New("ImageLabel", {
			Image = CustomImageManager.GetAsset(
				"TransparencyTexture"
			),
		...
		local ColorMenu = Library:AddContextMenu(
			Holder,
			UDim2.fromOffset(
				Info.Transparency and 256 or 234,
				0
			),
			function()
				return {
		...
		local ContextMenu = Library:AddContextMenu(
			Holder,
		...
		do
			...
			CreateButton("Paste color", function()
				ColorPicker:SetValueRGB(
					Library.CopiedColor[1],
					Library.CopiedColor[2]
				)
			end)
			if setclipboard then
				CreateButton("Copy Hex", function()
					setclipboard(
						tostring(ColorPicker.Value:ToHex())
					)
				end)
		...
		function ColorPicker:Display()
			...
			Holder.BorderColor3 = Library:GetDarkerColor(
				ColorPicker.Value
			)
			HolderTransparency.ImageTransparency =
				(1 - ColorPicker.Transparency)
			SatVipMap.BackgroundColor3 =
				Color3.fromHSV(ColorPicker.Hue, 1, 1)
			if TransparencyColor then
				TransparencyColor.BackgroundColor3 =
					ColorPicker.Value
			end
			SatVibCursor.Position = UDim2.fromScale(
				ColorPicker.Sat,
				1 - ColorPicker.Vib
			)
			HueCursor.Position = UDim2.fromScale(
				0.5,
				ColorPicker.Hue
			)
			if TransparencyCursor then
				TransparencyCursor.Position = UDim2.fromScale(
					0.5,
					ColorPicker.Transparency
				)
			end
		...
		function ColorPicker:SetValue(HSV, Transparency)
			...
			ColorPicker.Transparency =
				Info.Transparency and Transparency or 0
		...
		SatVipMap.InputBegan:Connect(function(Input)
			while IsDragInput(Input) do
				...
				ColorPicker.Sat =
					(LocationX - MinX) / (MaxX - MinX)
				ColorPicker.Vib =
					1
					- ((LocationY - MinY) / (MaxY - MinY))
				if ColorPicker.Sat ~= OldSat
					or ColorPicker.Vib ~= OldVib
				then
					ColorPicker:Update()
				end
				RunService.RenderStepped:Wait()
		...
		HueSelector.InputBegan:Connect(function(Input)
			while IsDragInput(Input) do
				...
				local Max =
					Min + HueSelector.AbsoluteSize.Y
				local Location = math.clamp(
					Mouse.Y,
					Min,
					Max
				)
		...
		if TransparencySelector then
			TransparencySelector.InputBegan:Connect(function(Input)
				while IsDragInput(Input) do
					local Min =
						TransparencySelector.AbsolutePosition.Y
					local Max =
						Min
						+ TransparencySelector.AbsoluteSize.Y
					local Location = math.clamp(
						Mouse.Y,
						Min,
						Max
...
do
	...
	function Funcs:CreateStatus(StatusName, InitialLines)
		...
		local Layout = New(
			"UIListLayout",
			{ Padding = UDim.new(0, 4), Parent = StatusHolder }
		)
		...
		function StatusObj:Update(TargetTitle, NewValue)
			if TrackedLabels[TargetTitle] then
				...
				TrackedLabels[TargetTitle].TextTransparency = (
						tostring(NewValue) == "None"
						or tostring(NewValue) == "0"
					)
					and 0.6
					or 0
			end
	...
	function Funcs:AddLabel(...)
		...
		local TextLabel = New("TextLabel", {
			...
			TextXAlignment = Groupbox.IsKeyTab
		...
		if Label.DoesWrap then
			...
			TextLabel:GetPropertyChangedSignal(
				"AbsoluteSize"
			):Connect(function()
	...
	function Funcs:AddButton(...)
		local function GetInfo(...)
			...
			if typeof(First) == "table"
				or typeof(Second) == "table"
			then
				local Params = typeof(First) == "table"
					and First
					or Second
				Info.Text = Params.Text or ""
				Info.Func = Params.Func
					or Params.Callback
					or function() end
		...
		local function CreateButton(Button)
			local Base = New("TextButton", {
				...
				BackgroundColor3 = Button.Disabled
					and "BackgroundColor"
					or "MainColor",
				Size = UDim2.fromScale(1, 1),
				Text = Button.Text,
		...
		local function InitEvents(Button)
			...
			Button.Base.InputBegan:Connect(function(Input)
				...
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					TweenService:Create(Button.Stroke, TweenInfo.new(0.05),
						{ Color = Library.Scheme.OutlineColor:Lerp(Library.Scheme.AccentColor, 0.15) }):Play()
				end
			end)
			...
			Button.Base.InputEnded:Connect(function(Input)
				if Button.Disabled or Button.Locked then return end
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					TweenService:Create(Button.Stroke, TweenInfo.new(0.15), { Color = Library.Scheme.OutlineColor })
						:Play()
				end
			end)
		...
		function Button:AddButton(...)
			...
			function SubButton:UpdateColors()
				...
				SubButton.Base.BackgroundColor3 = SubButton.Disabled
					and Library.Scheme.BackgroundColor
					or Library.Scheme.MainColor
				SubButton.Base.TextTransparency = SubButton.Disabled
					and 0.8
					or 0.4
				SubButton.Stroke.Transparency = SubButton.Disabled
					and 0.5
					or 0
				Library.Registry[SubButton.Base].BackgroundColor3 = SubButton.Disabled
					and "BackgroundColor"
					or "MainColor"
			end
			...
			if typeof(SubButton.Tooltip) == "string"
				or typeof(SubButton.DisabledTooltip) == "string"
			then
				SubButton.TooltipTable = Library:AddTooltip(
					SubButton.Tooltip,
					SubButton.DisabledTooltip,
					SubButton.Base
				)
				SubButton.TooltipTable.Disabled = SubButton.Disabled
			end
		...
		function Button:UpdateColors()
			...
			Button.Base.BackgroundColor3 = Button.Disabled
				and Library.Scheme.BackgroundColor
				or Library.Scheme.MainColor
			Button.Base.TextTransparency = Button.Disabled
				and 0.8
				or 0.4
			Button.Stroke.Transparency = Button.Disabled
				and 0.5
				or 0
			Library.Registry[Button.Base].BackgroundColor3 = Button.Disabled
				and "BackgroundColor"
				or "MainColor"
		end
		...
		if typeof(Button.Tooltip) == "string"
			or typeof(Button.DisabledTooltip) == "string"
		then
			Button.TooltipTable = Library:AddTooltip(
				Button.Tooltip,
	...
	function Funcs:AddCheckbox(Idx, Info)
		...
		function Toggle:Display()
			...
			if Toggle.Disabled then
				...
				CheckImage.ImageTransparency = Toggle.Value
					and 0.8
					or 1
		...
		function Toggle:SetValue(Value)
			...
			for _, Addon in Toggle.Addons do
				if Addon.Type == "KeyPicker"
					and Addon.SyncToggleState
				then
		...
		if typeof(Toggle.Tooltip) == "string"
			or typeof(Toggle.DisabledTooltip) == "string"
		then
			Toggle.TooltipTable = Library:AddTooltip(
				Toggle.Tooltip,
				Toggle.DisabledTooltip,
				Button
			)
			Toggle.TooltipTable.Disabled = Toggle.Disabled
		end
	...
	function Funcs:AddToggle(Idx, Info)
		...
		function Toggle:Display()
			...
			Switch.BackgroundTransparency = Toggle.Disabled
				and 0.75
				or 0
			SwitchStroke.Transparency = Toggle.Disabled
				and 0.75
				or 0
			Switch.BackgroundColor3 = Toggle.Value
				and Library.Scheme.AccentColor
				or Library.Scheme.MainColor
			SwitchStroke.Color = Toggle.Value
				and Library.Scheme.AccentColor
				or Library.Scheme.OutlineColor
			Library.Registry[Switch].BackgroundColor3 = Toggle.Value
				and "AccentColor"
				or "MainColor"
			Library.Registry[SwitchStroke].Color = Toggle.Value
				and "AccentColor"
				or "OutlineColor"
			if Toggle.Disabled then
				Label.TextTransparency = 0.8
				...
				Ball.BackgroundColor3 = Library:GetDarkerColor(
					Library.Scheme.FontColor
				)
				Library.Registry[Ball].BackgroundColor3 = function()
					return Library:GetDarkerColor(
						Library.Scheme.FontColor
					)
				end
				return
	...
	function Funcs:AddInput(Idx, Info)
		...
		Box.MouseEnter:Connect(function()
			...
			TweenService:Create(Box, Library.TweenInfo,
				{ BorderColor3 = Library.Scheme.OutlineColor:Lerp(Library.Scheme.AccentColor, 0.15) }):Play()
		end)
		...
		Box.Focused:Connect(function()
			...
			TweenService:Create(Box, Library.TweenInfo,
				{ BorderColor3 = Library.Scheme.OutlineColor:Lerp(Library.Scheme.AccentColor, 0.25) }):Play()
		end)
	...
	function Funcs:AddSlider(Idx, Info)
		...
		local Holder = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, Info.Compact and 16 or 34),
		...
		function Slider:UpdateColors()
			...
			if SliderLabel then
				SliderLabel.TextTransparency = Slider.Disabled
					and 0.8
					or 0
			end
			DisplayLabel.TextTransparency = Slider.Disabled
				and 0.8
				or 0
			Fill.BackgroundColor3 = Slider.Disabled
				and Library.Scheme.OutlineColor
				or Library.Scheme.AccentColor
			Library.Registry[Fill].BackgroundColor3 = Slider.Disabled
				and "OutlineColor"
				or "AccentColor"
		end
		...
		function Slider:Display()
			...
			if Info.FormatDisplayValue then
				CustomDisplayText = Info.FormatDisplayValue(
					Slider,
					Slider.Value
				)
			end
			...
			local X = (Slider.Value - Slider.Min)
		...
		function Slider:SetMax(Value)
			assert(
				Value > Slider.Min,
				"Max value cannot be less than the current min value."
			)
			Slider:SetValue(
				math.clamp(Slider.Value, Slider.Min, Value)
			)
			Slider.Max = Value
			Slider:Display()
		end
		...
		function Slider:SetMin(Value)
			assert(
				Value < Slider.Max,
				"Min value cannot be greater than the current max value."
			)
			Slider:SetValue(
				math.clamp(Slider.Value, Value, Slider.Max)
			)
			Slider.Min = Value
			Slider:Display()
		end
		...
		Bar.MouseEnter:Connect(function()
			...
			TweenService:Create(Bar, Library.TweenInfo,
				{ BorderColor3 = Library.Scheme.OutlineColor:Lerp(Library.Scheme.AccentColor, 0.3) }):Play()
		end)
		Bar.MouseLeave:Connect(function()
			if Slider.Disabled then return end
			TweenService:Create(Bar, Library.TweenInfo, { BorderColor3 = Library.Scheme.OutlineColor }):Play()
		end)
		...
		Bar.InputBegan:Connect(function(Input)
			...
			while IsDragInput(Input) do
				...
				local Scale = math.clamp(
					(Location - Bar.AbsolutePosition.X)
					/ Bar.AbsoluteSize.X,
					0,
					1
				)
		...
		if typeof(Slider.Tooltip) == "string"
			or typeof(Slider.DisabledTooltip) == "string"
		then
			Slider.TooltipTable = Library:AddTooltip(
				Slider.Tooltip,
				Slider.DisabledTooltip,
				Bar
			)
			Slider.TooltipTable.Disabled = Slider.Disabled
		end
	...
	function Funcs:AddDropdown(Idx, Info)
		...
		local MenuTable = Library:AddContextMenu(
			...
			function() return { 0, Display.AbsoluteSize.Y + 4 } end,
			2,
			function(Active)
				Display.TextTransparency = (Active and SearchBox) and 1 or (Dropdown.Disabled and 0.8 or 0)
				ArrowImage.ImageTransparency = Active and 0 or 0.5
				ArrowImage.Rotation = Active and 180 or 0
				TweenService:Create(Display, Library.TweenInfo,
					{
						BorderColor3 = Active and Library.Scheme.OutlineColor:Lerp(Library.Scheme.AccentColor, 0.4) or
		...
		Display.MouseLeave:Connect(function()
			if Dropdown.Disabled or MenuTable.Active then return end
			TweenService:Create(Display, Library.TweenInfo, { BorderColor3 = Library.Scheme.OutlineColor }):Play()
		end)

		function Dropdown:RecalculateListSize(Count)
			local Y = math.clamp((Count or GetTableSize(Dropdown.Values)) * 24, 0, Info.MaxVisibleDropdownItems * 24)
			MenuTable:SetSize(function() return UDim2.fromOffset(Display.AbsoluteSize.X / Library.DPIScale, Y) end)
		end
		...
		function Dropdown:BuildDropdownList()
			...
			for Button, _ in Buttons do Button:Destroy() end
			table.clear(Buttons)
			local Count = 0
			for _, Value in Values do
				local __DARKLUA_CONTINUE_76 = false
				repeat
					if SearchBox and not string.find(tostring(Value):lower(), SearchBox.Text:lower(), 1, true) then
						__DARKLUA_CONTINUE_76 = true
						break
					end
					Count = Count + 1
					...
					__DARKLUA_CONTINUE_76 = true
				until true
				if not __DARKLUA_CONTINUE_76 then break end
			end
		...
		function Dropdown:SetValue(Value)
			if Info.Multi then
				...
				if table.find(Dropdown.Values, Value) then Dropdown.Value = Value elseif not Value then Dropdown.Value = nil end
			end
			Dropdown:Display()
		...
		function Dropdown:SetValues(Values)
			Dropdown.Values = Values; Dropdown:BuildDropdownList()
		end
		...
		function Dropdown:SetDisabledValues(DisabledValues)
			Dropdown.DisabledValues = DisabledValues; Dropdown:BuildDropdownList()
		end
		...
		function Dropdown:SetText(Text)
			Dropdown.Text = Text
			Holder.Size = UDim2.new(1, 0, 0, Text and 42 or 24)
		...
		if next(Defaults) then
			for i = 1, #Defaults do
				...
				if Info.Multi then
					...
					Dropdown.Value = Dropdown.Values
						[Index]
				end
	...
	function Funcs:AddViewport(Idx, Info)
		...
		local function FocusCamera()
			...
			Viewport.Camera.CFrame = CFrame.new(ModelPosition + Vector3.new(0, MaxExtent / 2, CameraDistance),
				ModelPosition)
		end
		...
		local function SetScrollingEnabled(state)
			if Groupbox.Tab and Groupbox.Tab.Sides then
		...
		Library:GiveSignal(UserInputService.InputChanged:Connect(function(input)
			if Library.Unloaded or not Viewport.Interactive or not Dragging or Pinching then return end
		...
		Library:GiveSignal(UserInputService.TouchPinch:Connect(function(touchPositions, scale, velocity, state)
			if Library.Unloaded or not Viewport.Interactive or not Library:MouseIsOverFrame(ViewportFrame, touchPositions[1]) then return end
	...
	function Funcs:AddImage(Idx, Info)
		...
		local Icon = Library:GetCustomIcon(
		...
		function Image:SetRectOffset(RectOffset)
			assert(
				typeof(RectOffset) == "Vector2",
				"RectOffset must be a Vector2 value."
			)
		...
		function Image:SetRectSize(RectSize)
			assert(
				typeof(RectSize) == "Vector2",
				"RectSize must be a Vector2 value."
			)
		...
		function Image:SetScaleType(ScaleType)
			assert(
				typeof(ScaleType) == "EnumItem"
				and ScaleType:IsA("ScaleType"),
		...
		function Image:SetTransparency(Transparency)
			assert(
				...
				"Transparency must be a number between 0 and 1."
			)
			assert(
				Transparency >= 0 and Transparency <= 1,
	...
	function Funcs:AddVideo(Idx, Info)
		...
		function Video:SetLooped(Looped)
			assert(
				typeof(Looped) == "boolean",
				"Looped must be a boolean."
			)
	...
	function Funcs:AddUIPassthrough(Idx, Info)
		...
		assert(
			typeof(Info.Instance) == "Instance"
			and Info.Instance:IsA("GuiBase2d"),
		...
		function Passthrough:SetHeight(Height)
			assert(
				typeof(Height) == "number" and Height > 0,
				"Height must be a number greater than 0."
			)
	...
	function Funcs:AddList(Idx, Info)
		...
		local function UpdateRowHighlight(key)
			...
			local TargetBg = selected
				and Library.Scheme.AccentColor
				or Library.Scheme.MainColor
			local TargetText = selected
				and Library.Scheme.BackgroundColor
		...
		local function RenderItems()
			...
			for i, item in List.Items do
				...
				local BgColor = selected
					and Library.Scheme.AccentColor
					or Library.Scheme.MainColor
				local TxtColor = selected
					and Library.Scheme.BackgroundColor
					or Library.Scheme.FontColor
		...
		function List:SetDisabled(disabled)
			...
			ListFrame.ScrollBarImageTransparency = disabled
				and 0.5
				or 0
		end
	...
	function Funcs:AddDependencyBox()
		...
		function Depbox:Resize()
			DepboxContainer.Size = UDim2.new(
				...
				0,
				0,
				DepboxList.AbsoluteContentSize.Y
				/ Library.DPIScale
			)
			Groupbox:Resize()
		end
		...
		DepboxList:GetPropertyChangedSignal(
			"AbsoluteContentSize"
		):Connect(function()
			if not Depbox.Visible then
				return
		...
		DepboxContainer:GetPropertyChangedSignal(
			"Visible"
		):Connect(function()
			Depbox:Resize()
		end)
	...
	function Funcs:AddDependencyGroupbox()
		...
		function DepGroupbox:Resize()
			DepGroupboxContainer.Size = UDim2.new(
				1,
				0,
				0,
				(DepGroupboxList.AbsoluteContentSize.Y
					/ Library.DPIScale)
				+ 18
			)
		end
	...
	function Funcs:AddTabbox()
		...
		local InnerTabTweenInfo = TweenInfo.new(
			0.2,
			Enum.EasingStyle.Quad,
			Enum.EasingDirection.Out
		)
		function InnerTabbox:AddTab(Name, IconName)
			...
			function Tab:Show()
				...
				if ButtonIcon then
					TweenService:Create(ButtonIcon, InnerTabTweenInfo,
						{ ImageTransparency = 0, ImageColor3 = Library.Scheme.AccentColor }):Play()
				end
				TweenService:Create(Indicator, InnerTabTweenInfo,
					{ BackgroundTransparency = 0, Size = UDim2.new(0.6, 0, 0, 2) }):Play()
			...
			function Tab:Hide()
				if ButtonIcon then
					TweenService:Create(ButtonIcon, InnerTabTweenInfo,
						{ ImageTransparency = 0.5, ImageColor3 = Library.Scheme.FontColor }):Play()
				end
				TweenService:Create(Indicator, InnerTabTweenInfo,
					{ BackgroundTransparency = 1, Size = UDim2.new(0, 0, 0, 2) }):Play()
			...
			function Tab:Resize()
				...
				TabboxWrapper.Size = UDim2.new(1, 0, 0, 36 + ContentHeight)
				Groupbox:Resize()
			end
			...
			Button.MouseButton1Click:Connect(function() Tab:Show() end)
...
function Library:Notify(...)
	...
	local Holder = New("Frame", {
		AutomaticSize = Enum.AutomaticSize.Y,
		BackgroundColor3 = "MainColor",
		Position = Library.NotifySide:lower() == "left"
	...
	local Title, Desc, TitleX, DescX, TimerFill = nil, nil, 0, 0, nil
	if Data.Title then
		Title = New("TextLabel", {
	...
	function Data:Resize()
		...
		if Title then
			local X, Y = Library:GetTextBounds(
				...
				Title.FontFace,
				Title.TextSize,
				(NotificationArea.AbsoluteSize.X / Library.DPIScale)
				- 24
				- ExtraWidth
				- IconWidth
			)
			...
			TitleX = X + IconWidth
			TitleContainer.Size = UDim2.fromOffset(
				TitleX,
				math.max(Y, IconLabel and 16 or 0)
			)
		end
		if Desc then
			local X, Y = Library:GetTextBounds(
		...
		FakeBackground.Size = UDim2.fromOffset(
			math.max(TitleX, DescX) + 24 + ExtraWidth,
			0
		)
	end
	...
	function Data:ChangeStep(NewStep)
		if TimerFill and Data.Steps then
			...
			TimerFill.Size = UDim2.fromScale(
				NewStep / Data.Steps,
				1
			)
		end
...
function Library:CreatePredictionHUD(Info)
	local HUD = {
		Visible = true,
		Rows = {}
	}
	...
	New("UICorner", { CornerRadius = UDim.new(0, 6), Parent = PredictCard })
	...
	local Logo = New("ImageLabel", {
		...
		Size = UDim2.fromOffset(20, 20),
		BackgroundTransparency = 1,
		Image = "rbxassetid://119236006737744",
		ZIndex = 11002,
		Parent = HeaderFrame,
	})
	...
	local TitleLabel = New("TextLabel", {
		...
		BackgroundTransparency = 1,
		Text = Info.Title or "PREDICTION HUD",
	...
	local LiveDot = New("Frame", {
		...
		Size = UDim2.fromOffset(6, 6),
		BackgroundColor3 = Color3.fromRGB(220, 30, 45),
		ZIndex = 11002,
		Parent = HeaderFrame,
	})
	New("UICorner", { CornerRadius = UDim.new(1, 0), Parent = LiveDot })
	local DotGlow = New("UIStroke", {
		Color = Color3.fromRGB(220, 30, 45),
		Thickness = 2,
		Transparency = 0.5,
		Parent = LiveDot
	})

	local pulseTween = TweenService:Create(LiveDot,
		TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
			BackgroundTransparency = 0.6
		})
	pulseTween:Play()

	Library:MakeDraggable(PredictCard, HeaderFrame, true)

	-- 3. Wadah Baris Sub-Card
	local RowsContainer = New("Frame", {
		Name = "RowsContainer",
	...
	New("UIPadding", { PaddingBottom = UDim.new(0, 8), Parent = RowsContainer }) -- Jeda bawah estetik

	local function createRow(categoryName, defaultValue)
		local RowFrame = New("Frame", {
			Name = "Row_" .. categoryName,
			Size = UDim2.new(1, 0, 0, 30),
		...
		New("UICorner", { CornerRadius = UDim.new(0, 4), Parent = RowFrame })
		local Stroke = New("UIStroke",
			{
				Color = Color3.fromRGB(44, 44, 44),
				Thickness = 1,
		...
		HUD.Rows[categoryName] = Value
	end

	if Info.Rows then
		for _, rName in ipairs(Info.Rows) do
			createRow(rName, "--")
		end
	end

	function HUD:Update(category, newValue)
		local targetLabel = HUD.Rows[category]
		if targetLabel then
			targetLabel.Text = tostring(newValue)

			-- Kedipkan teks menyala putih sesaat sebagai umpan balik visual mewah
...
function Library:CreateWindow(WindowInfo)
	...
	local function runKeySystem()
		...
		New("UIStroke",
			{
				Color = Library.Scheme.AccentColor or Color3.fromRGB(220, 30, 45),
				Thickness = 1,
		...
		local function createGatewayButton(name, logoId, link)
			...
			local label = New("TextLabel", {
				...
				TextColor3 = Color3.fromRGB(245, 245, 245),
				TextSize = 13,
				FontFace = Library.Scheme.Font or
			...
			btn.MouseLeave:Connect(function()
				...
				TweenService:Create(icon, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
					{ Position = UDim2.new(0, 10, 0.5, 0) }):Play()
				TweenService:Create(label, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
					{ Position = UDim2.new(0, 38, 0.5, 0) }):Play()
			end)
	...
	Library.OriginalMinSize = Vector2.new(
		math.min(Library.OriginalMinSize.X, MaxX),
		math.min(Library.OriginalMinSize.Y, MaxY)
	)
	...
	WindowInfo.CornerRadius = math.min(
	...
	WindowInfo.MinSidebarWidth = math.max(
		64,
		WindowInfo.MinSidebarWidth
	)
	WindowInfo.SidebarCompactWidth = math.max(
		48,
		WindowInfo.SidebarCompactWidth
	)
	WindowInfo.SidebarCollapseThreshold = math.clamp(
		WindowInfo.SidebarCollapseThreshold,
		0.1,
		0.9
	)
	WindowInfo.CompactWidthActivation = math.max(
		48,
	...
	local IsDefaultSearchbarSize =
		WindowInfo.SearchbarSize == UDim2.fromScale(1, 1)
	...
	do
		...
		local fpsCount, nextUpdate, currentFps =
			0,
			os.clock() + 0.5,
			60
		RunService.RenderStepped:Connect(function()
			...
			if now >= nextUpdate then
				...
				fpsCount = 0
				nextUpdate = now + 0.5
				local pingVal = math.round(
					LocalPlayer:GetNetworkPing() * 1000
				)
				local gameVer = tostring(
					game.PlaceVersion
				)
				local accentHex =
		...
		if MoveIcon then
			New("ImageLabel", {
				...
				ImageRectOffset =
					MoveIcon.ImageRectOffset,
				ImageRectSize =
					MoveIcon.ImageRectSize,
				Position = UDim2.new(
					1,
					-10,
					0.5,
		...
		New("ImageLabel", {
			Image = ResizeIcon and ResizeIcon.Url
				or "",
			ImageColor3 = "FontColor",
			ImageRectOffset = ResizeIcon
				and ResizeIcon.ImageRectOffset
				or Vector2.zero,
			ImageRectSize = ResizeIcon
				and ResizeIcon.ImageRectSize
				or Vector2.zero,
			ImageTransparency = 0.5,
			Position = UDim2.fromOffset(2, 2),
			Size = UDim2.new(1, -4, 1, -4),
	...
	function Window:SetCornerRadius(Radius)
		...
		for _, UICorner in Library.Corners do
			if UICorner.CornerRadius.Offset
				== Library.CornerRadius / 2
			then
		...
		ResizeButton.Position = UDim2.new(
			1,
			-Radius / 4,
			0,
			0
		)
		BottomBackground.Size = UDim2.new(
			1,
			0,
			0,
			20 + Radius
		...
		for _, Tab in Library.Tabs do
			local __DARKLUA_CONTINUE_102 = false
			repeat
				if Tab.IsKeyTab then
					__DARKLUA_CONTINUE_102 = true
					break
				end
				for _, Tabbox in Tab.Tabboxes do
				...
				__DARKLUA_CONTINUE_102 = true
			until true
			if not __DARKLUA_CONTINUE_102 then break end
		end
	end
	...
	local function ApplyCompact()
		...
		for _, Button in Library.TabButtons do
			local __DARKLUA_CONTINUE_104 = false
			repeat
				if not Button.Icon then
					__DARKLUA_CONTINUE_104 = true
					break
				end
				...
				__DARKLUA_CONTINUE_104 = true
			until true
			if not __DARKLUA_CONTINUE_104 then break end
		end
	end
	...
	function Window:SetCompact(State)
		Window:SetSidebarWidth(
			State and WindowInfo.SidebarCompactWidth
			or LastExpandedWidth
		)
	end
	...
	function Window:SetSidebarWidth(Width)
		...
		DividerLine.Position = UDim2.fromOffset(
			Width,
			0
		)
		TitleHolder.Size = UDim2.new(
			0,
			Width,
			1,
			0
		)
		RightWrapper.Size = UDim2.new(
			1,
			-Width - 60,
			1,
			-16
	...
	function Window:AddTab(...)
		...
		do
			...
			TabLabel = New("TextLabel", { Text = "", Visible = false, Parent = TabButton })
			...
			LeftList:GetPropertyChangedSignal(
				"AbsoluteContentSize"
			):Connect(function()
				TabLeft.CanvasSize = UDim2.new(
					0,
					0,
					0,
			...
			RightList:GetPropertyChangedSignal(
				"AbsoluteContentSize"
			):Connect(function()
				TabRight.CanvasSize = UDim2.new(
					0,
					0,
					0,
		...
		do
			...
			WarningBoxOutline, WarningBoxShadowOutline =
				Library:AddOutline(WarningBox)
			WarningBoxScrollingFrame = New(
				"ScrollingFrame",
				{
					BackgroundTransparency = 1,
					BorderSizePixel = 0,
		...
		function Tab:UpdateWarningBox(Info)
			if typeof(Info.IsNormal) == "boolean" then
				Tab.WarningBox.IsNormal =
					Info.IsNormal
			end
			if typeof(Info.LockSize) == "boolean" then
				Tab.WarningBox.LockSize =
					Info.LockSize
			end
			...
			WarningBoxHolder.Visible =
				Tab.WarningBox.Visible
			WarningTitle.Text = Tab.WarningBox.Title
			WarningText.Text = Tab.WarningBox.Text
			Tab:Resize(true)
			WarningBox.BackgroundColor3 =
				Tab.WarningBox.IsNormal == true
				and Library.Scheme.BackgroundColor
				or Color3.fromRGB(127, 0, 0)
			WarningBoxShadowOutline.Color =
				Tab.WarningBox.IsNormal == true
				and Library.Scheme.DarkColor
				or Color3.fromRGB(85, 0, 0)
			WarningBoxOutline.Color =
				Tab.WarningBox.IsNormal == true
				and Library.Scheme.OutlineColor
				or Color3.fromRGB(255, 50, 50)
			WarningTitle.TextColor3 =
				Tab.WarningBox.IsNormal == true
			...
			.BackgroundColor3 = function()
				return Tab.WarningBox.IsNormal
					== true
					and Library.Scheme
					.BackgroundColor
					or Color3.fromRGB(127, 0, 0)
			end
			Library.Registry[WarningBoxShadowOutline]
			.Color = function()
				return Tab.WarningBox.IsNormal
					== true
					and Library.Scheme.DarkColor
			...
			Library.Registry[WarningBoxOutline]
			.Color = function()
				return Tab.WarningBox.IsNormal
					== true
					and Library.Scheme
		...
		function Tab:RefreshSides()
			local Offset = WarningBoxHolder.Visible
		...
		function Tab:Resize(ResizeWarningBox)
			if ResizeWarningBox then
				local MaximumSize = math.floor(
					TabContainer.AbsoluteSize.Y / 3.25
				)
				...
				local YBox = 24 + YText
				if Tab.WarningBox.LockSize == true
					and YBox >= MaximumSize
				then
					WarningBoxScrollingFrame.CanvasSize =
						UDim2.fromOffset(0, YBox)
					YBox = MaximumSize
				else
					WarningBoxScrollingFrame.CanvasSize =
						UDim2.fromOffset(0, 0)
				end
				WarningText.Size = UDim2.new(
					1,
					-4,
					0,
					YText
				)
				WarningBox.Size = UDim2.new(
					1,
					-5,
					0,
					YBox + 4
		...
		function Tab:AddGroupbox(Info)
			...
			do
				...
				GroupboxTitleLine = Library:MakeLine(
					GroupboxHolder,
					{
						Position =
							UDim2.fromOffset(
				...
				local BoxIcon = Library:GetCustomIcon(
					Info.IconName
				)
				if BoxIcon then
					New("ImageLabel", {
						Image = BoxIcon.Url,
						ImageColor3 = BoxIcon.Custom
							and "WhiteColor"
							or "AccentColor",
						ImageRectOffset =
				...
				local ChevronIcon = Library:GetIcon(
					"chevron-down"
				) or ArrowIcon
				MinimizeBtn = New("ImageButton", {
					...
					Image = ChevronIcon
						and ChevronIcon.Url
						or "",
					ImageRectOffset = ChevronIcon
						and ChevronIcon.ImageRectOffset
						or Vector2.zero,
					ImageRectSize = ChevronIcon
						and ChevronIcon.ImageRectSize
						or Vector2.zero,
			...
			function Groupbox:Resize()
				...
				GroupboxHolder.Size = UDim2.new(
					...
					0,
					(
						GroupboxList.AbsoluteContentSize.Y
						/ Library.DPIScale
					) + 49
				)
			end
	...
	function Window:AddKeyTab(...)
		...
		function Tab:AddKeyBox(Callback)
			...
			Button.InputBegan:Connect(function(Input)
				...
				if not Library:MouseIsOverFrame(
						Button,
						Input.Position
					) then
					return
	...
	function Window:AddDialog(Idx, Info)
		...
		if Info.Icon then
			local ParsedIcon = Library:GetCustomIcon(
				Info.Icon
			)
			if ParsedIcon then
				local IconImg = New("ImageLabel", {
					...
					ImageColor3 = "FontColor",
					ImageRectOffset =
						ParsedIcon.ImageRectOffset,
					ImageRectSize =
						ParsedIcon.ImageRectSize,
					LayoutOrder = 1,
					ZIndex = 9002,
					Parent = TitleRow,
				})
		...
		local DescriptionLabel = New("TextLabel", {
			...
			TextSize = 14,
			TextTransparency = Info.DescriptionColor
				and 0
				or 0.2,
			TextXAlignment =
				Enum.TextXAlignment.Left,
		...
		function Dialog:Resize()
			...
			local TotalButtonWidth, ButtonCount, HasButtons =
				0,
				0,
				false
			...
			for _, BtnWrap in FooterButtonsList do
				...
				TotalButtonWidth = TotalButtonWidth +
					BtnWrap.Container.Size.X.Offset
			end
			...
			elseif HasButtons then
				local RequiredWidth = TotalButtonWidth
					+ ((ButtonCount - 1) * 8)
					+ 30
				TargetWidth = math.max(
					MinWidth,
					math.min(RequiredWidth, MaxWidth)
				)
			end
			DialogFrame.Size = UDim2.fromOffset(
				TargetWidth,
				0
			)

			local maxDialogHeight =
				MainFrame.AbsoluteSize.Y * 0.85
			local contentHeight =
				InnerLayout.AbsoluteContentSize.Y + 30
			local innerHeight = math.min(
				contentHeight,
				maxDialogHeight
			)
			InnerContainer.Size = UDim2.new(
				1,
				0,
				0,
				innerHeight
			)
			InnerContainer.ScrollBarThickness =
				contentHeight > maxDialogHeight
				and 4
				or 0

			local _, DescY = Library:GetTextBounds(
				DescriptionLabel.Text,
				Library.Scheme.Font,
				14,
				TargetWidth - 30
			)
			DescriptionLabel.Size = UDim2.new(
				1,
				0,
				0,
				DescY
		...
		DialogOverlay.MouseButton1Click:Connect(
			function()
		...
		function Dialog:RemoveFooterButton(ButtonIdx)
			if FooterButtonsList[ButtonIdx] then
				FooterButtonsList[ButtonIdx]
					.Container:Destroy()
				FooterButtonsList[ButtonIdx] = nil
			end
		end

		function Dialog:SetButtonDisabled(
			ButtonIdx,
			Disabled
		)
			if FooterButtonsList[ButtonIdx]
				and type(
					FooterButtonsList[ButtonIdx]
					.SetDisabled
				) == "function"
			then
				FooterButtonsList[ButtonIdx]:SetDisabled(
					Disabled
				)
			end
		end
		...
		function Dialog:SetButtonOrder(ButtonIdx, Order)
			if FooterButtonsList[ButtonIdx]
				and FooterButtonsList[ButtonIdx]
				.Container
			then
				FooterButtonsList[ButtonIdx]
		...
		function Dialog:SetButtonText(ButtonIdx, Text)
			...
			if BtnWrap and BtnWrap.Label then
				...
				local LabelX, _ = Library:GetTextBounds(
					Text,
					Library.Scheme.Font,
					14,
					250
				)
				BtnWrap.Container.Size =
					UDim2.fromOffset(
						LabelX + 30,
						26
					)
				BtnWrap.TextBtn.Size =
					UDim2.fromOffset(
						LabelX + 30,
						26
					)
		...
		function Dialog:SetButtonVariant(
			ButtonIdx,
			NewVariant
		)
			local BtnWrap =
			...
			BtnWrap.TextBtn.BackgroundColor3 =
				BtnColor
			BtnWrap.Label.TextColor3 = TextColor
		end

		function Dialog:AddFooterButton(
			ButtonIdx,
			ButtonInfo
		)
			...
			local BtnColor, BtnOutline, Variant =
				"MainColor",
				"OutlineColor",
				ButtonInfo.Variant or "Primary"
			...
			elseif Variant == "Destructive" then
				BtnColor, BtnOutline = Color3.fromRGB(
						220,
						38,
						38
					),
			...
			local TextBtn = New("TextButton", {
				...
				BackgroundTransparency = WaitTime > 0
					and 0.5
					or 0,
				Size = UDim2.fromOffset(0, 26),
				Text = "",
			...
			ButtonContainer.Size = UDim2.fromOffset(
				LabelX + 30,
				26
			)
			TextBtn.Size = UDim2.fromOffset(
				LabelX + 30,
				26
			)
			...
			local ActiveColor = typeof(BtnColor)
				== "Color3"
				and BtnColor
				or Library.Scheme[BtnColor]
			local HoverColor = Variant == "Ghost"
				and Library.Scheme.MainColor
				or Library:GetBetterColor(
					ActiveColor,
					10
				)
	...
	if Library.IsMobile then
		local LockButton = Library:AddDraggableButton(
			"Lock",
			function(self)
				Library.CantDragForced =
					not Library.CantDragForced
				self:SetText(
					Library.CantDragForced
					and "Unlock"
					or "Lock"
		...
		if WindowInfo.MobileButtonsSide == "Right" then
			LockButton.Button.Position = UDim2.new(
				1,
				-6,
				0,
				46
			)
			LockButton.Button.AnchorPoint =
				Vector2.new(1, 0)
		else
			LockButton.Button.Position =
				UDim2.fromOffset(6, 46)
		end
	end
	...
	Library:GiveSignal(
		UserInputService.InputBegan:Connect(
			function(Input)
				...
				if (
						typeof(Library.ToggleKeybind)
						== "table"
						and Library.ToggleKeybind.Type
						== "KeyPicker"
...
function Library:Confirm(Info)
	assert(
		Library.Window,
		"Library:Confirm requires a window to be created first"
	)
	...
	local ConfirmIdx = "__Confirm_"
		.. tostring(tick())
	local ConfirmDialog
	ConfirmDialog = Library.Window:AddDialog(
		ConfirmIdx,
		{
...
local function OnPlayerChange()
	...
	local PlayerList, ExcludedPlayerList =
		GetPlayers(),
		GetPlayers(true)
	for _, Dropdown in Options do
		if Dropdown.Type == "Dropdown"
			and Dropdown.SpecialType == "Player"
		then
			Dropdown:SetValues(
				Dropdown.ExcludeLocalPlayer
				and ExcludedPlayerList
				or PlayerList
			)
		end
	end
...
function Library:CreateNotification(Data)
	Library:Notify({
		Title = Data.Title,
		Description = Data.Content,
		Time = Data.Duration or 4,
	})
end
