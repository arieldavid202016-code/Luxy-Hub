...
do
    ...
    function CustomImageManager.DownloadAsset(
        AssetName: string,
        ForceRedownload: boolean?
    )
...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromRGB(165, 39, 0),
        OutlineColor = Color3.fromRGB(49, 49, 49),
        ...
        RedColor = Color3.fromRGB(255, 119, 0),
        DarkColor = Color3.fromRGB(12, 12, 12),
...
function Library:GiveSignal(
    Connection: RBXScriptConnection | RBXScriptSignal
)
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        function KeyPicker:SetValue(Data)
            ...
            KeyPicker.Modifiers = VerifyModifiers(
                if typeof(Modifiers) == "table"
                    then Modifiers
                    else KeyPicker.Modifiers
            )
        ...
        Picker.MouseButton1Click:Connect(function()
            ...
            ActiveModifiers = if Input.KeyCode == Enum.KeyCode.Escape
                or Key == "Unknown"
                then {}
                else ActiveModifiers
            KeyPicker.Toggled = false
            KeyPicker:SetValue({
                Key,
                KeyPicker.Mode,
...
do
            ...
            Button.Base.MouseButton1Click:Connect(function()
                ...
                if Button.DoubleClick then
                    ...
                    local Clicked = WaitForEvent(
                        Button.Base.MouseButton1Click,
                        0.5
                    )
                    Button.Base.Text = Button.Text
                    Button.Base.TextColor3 = Button.Risky
                        and Library.Scheme.RedColor
                        or Library.Scheme.FontColor
                    Library.Registry[Button.Base].TextColor3 = Button.Risky
                        and "RedColor"
                        or "FontColor"
                    if Clicked then
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        local Box = New("TextBox", {
            ...
            BorderColor3 = "OutlineColor",
            BorderSizePixel = 1,
            ClearTextOnFocus = not Input.Disabled
                and Input.ClearTextOnFocus,
        ...
        function Input:SetDisabled(Disabled: boolean)
            ...
            Box.ClearTextOnFocus = not Input.Disabled
                and Input.ClearTextOnFocus
        ...
        if typeof(Input.Tooltip) == "string"
            or typeof(Input.DisabledTooltip) == "string"
        then
            Input.TooltipTable = Library:AddTooltip(
                Input.Tooltip,
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local MenuTable = Library:AddContextMenu(
            ...
            function(Active: boolean)
                Display.TextTransparency = (Active and SearchBox)
                    and 1
                    or 0
        ...
        function Dropdown:UpdateColors()
            ...
            ArrowImage.ImageTransparency = Dropdown.Disabled
                and 0.8
                or MenuTable.Active and 0 or 0.5
        end
        ...
        function Dropdown:BuildDropdownList()
            ...
            for _, Value in Values do
                ...
                function Table:UpdateButton()
                    ...
                    Button.BackgroundTransparency = Selected
                        and 0
                        or 1
                    Button.TextTransparency = IsDisabled
                        and 0.8
                        or Selected and 0 or 0.5
                end
            ...
            if Info.Multi then
                ...
                for Val, Active in Value or {} do
                    if typeof(Active) ~= "boolean" then
                        Table[Active] = true
                    elseif Active
                        and table.find(Dropdown.Values, Val)
                    then
                        Table[Val] = true
                    end
                end
        ...
        if SearchBox then
            SearchBox:GetPropertyChangedSignal("Text")
                :Connect(Dropdown.BuildDropdownList)
        end
        ...
        if typeof(Dropdown.Tooltip) == "string"
            or typeof(Dropdown.DisabledTooltip) == "string"
        then
            Dropdown.TooltipTable = Library:AddTooltip(
                Dropdown.Tooltip,
                Dropdown.DisabledTooltip,
                Display
            )
            Dropdown.TooltipTable.Disabled = Dropdown.Disabled
        end
    ...
    function Funcs:AddViewport(Idx, Info)
        ...
        local Viewport = {
            ...
            Camera = if not Info.Camera
                then Instance.new("Camera")
                else Info.Camera,
        ...
        assert(
            typeof(Viewport.Object) == "Instance"
                and (
                    Viewport.Object:IsA("BasePart")
                    or Viewport.Object:IsA("Model")
                ),
            "Instance must be a BasePart or Model."
        )
        assert(
            typeof(Viewport.Camera) == "Instance"
                and Viewport.Camera:IsA("Camera"),
        ...
        local function FocusCamera()
            ...
            local ModelPosition = Viewport.Object:GetPivot()
                .Position
            Viewport.Camera.CFrame = CFrame.new(
                ModelPosition
                    + Vector3.new(
                        0,
                        MaxExtent / 2,
            ...
            UserInputService.InputChanged:Connect(function(input)
                ...
                if input.UserInputType
                    ...
                    or input.UserInputType
                        == Enum.UserInputType.Touch
                then
                    local MouseDelta = input.Position
                        - LastMousePos
                    LastMousePos = input.Position
                    local Position = Viewport.Object
                        :GetPivot()
                        .Position
                    local Camera = Viewport.Camera
                    local RotationY = CFrame.fromAxisAngle(
                        Vector3.new(0, 1, 0),
                        -MouseDelta.X * 0.01
                    )
                    Camera.CFrame = CFrame.new(Position)
                        ...
                        * CFrame.new(-Position)
                        * Camera.CFrame
                    local RotationX = CFrame.fromAxisAngle(
                        Camera.CFrame.RightVector,
                        -MouseDelta.Y * 0.01
                    )
                    local PitchedCFrame = CFrame.new(Position)
                        * RotationX
                        * CFrame.new(-Position)
                        * Camera.CFrame
        ...
        ViewportFrame.InputChanged:Connect(function(input)
            ...
            then
                ...
                Viewport.Camera.CFrame = Viewport.Camera.CFrame
                    + Viewport.Camera.CFrame.LookVector
                        * ZoomAmount
            end
        end)
        Library:GiveSignal(
            UserInputService.TouchPinch:Connect(function(
                ...
                if not Viewport.Interactive
                    or not Library:MouseIsOverFrame(
                        ViewportFrame,
                        touchPositions[1]
                    )
                then
                ...
                if state == Enum.UserInputState.Begin then
                    Pinching = true
                    Dragging = false
                    LastPinchDist = (touchPositions[1]
                        - touchPositions[2]).Magnitude
                elseif state == Enum.UserInputState.Change then
                    local currentDist = (touchPositions[1]
                        - touchPositions[2]).Magnitude
                    local delta = (currentDist - LastPinchDist)
                        * 0.1
    ...
    function Funcs:AddTabbox()
        ...
        function InnerTabbox:AddTab(Name, IconName)
            ...
            function Tab:Resize()
                ...
                local ContentHeight = TabList.AbsoluteContentSize.Y
                    / Library.DPIScale
                    + 11
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        SearchBox = New("TextBox", {
            ...
            TextXAlignment = Enum.TextXAlignment.Left,
            Visible = not (
                WindowInfo.DisableSearch or false
            ),
            Parent = RightWrapper,
        })
        table.insert(
        ...
        if SearchIcon then
            New("ImageLabel", {
                ...
                ImageColor3 = "FontColor",
                ImageRectOffset =
                    SearchIcon.ImageRectOffset,
                ImageRectSize =
                    SearchIcon.ImageRectSize,
                ImageTransparency = 0.5,
                Position = UDim2.new(
                    0,
                    -20,
        ...
        do
            ...
            if Icon then
                TabIcon = New("ImageLabel", {
                    ...
                    ScaleType = Enum.ScaleType.Fit,
                    Size = UDim2.fromScale(1, 1),
                    SizeConstraint = IsCompact
                        and Enum.SizeConstraint
                            .RelativeXY
                        or Enum.SizeConstraint
                            .RelativeYY,
                    Parent = TabButton,
                })
            end
    ...
    function Window:AddKeyTab(...)
        ...
        Icon = if Icon == "key" then KeyIcon
            else Library:GetCustomIcon(Icon)
        do
            TabButton = New("TextButton", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 0, 40),
...
local function MakeSmartGroupbox(gb)
    ...
    gb.CreateStatus = function(
        ...
        table.insert(
            Library.Scales,
            New("UIScale", { Parent = StatusHolder })
        )
        ...
        function StatusObj:Update(
            ...
            if TrackedLabels[TargetTitle] then
                TrackedLabels[TargetTitle].Text =
                    tostring(NewValue)
                TrackedLabels[TargetTitle]
                    .TextTransparency = (
                    tostring(NewValue) == "None"
                        or tostring(NewValue) == "0"
                )
...
function Library:CreateWindow(Info)
    ...
    win.CreateTab = function(
        ...
        tab.CreateGroup = function(
            ...
            local gb
            if leftSide then
                gb = tab:AddLeftGroupbox(
                    GroupName,
                    GroupIcon
                )
                leftSide = false
            else
                gb = tab:AddRightGroupbox(
                    GroupName,
                    GroupIcon
                )
                leftSide = true
